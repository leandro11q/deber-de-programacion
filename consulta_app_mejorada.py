"""
Consulta App Mejorada (GUI + CLI con fallback)
------------------------------------------------
Este script ofrece una aplicación de consulta con autenticación de usuarios y
una pequeña base de conocimiento (CRUD). 

- **GUI (CustomTkinter/Tkinter)** si el entorno lo permite.
- **CLI interactiva** automáticamente si el entorno **no** tiene Tkinter,
  resolviendo el error: `ModuleNotFoundError: No module named 'tkinter'`.
- **Pruebas unitarias** integradas ejecutables con `--run-tests`.

Uso rápido:
- GUI (si hay soporte): `python app.py`
- Forzar CLI: `python app.py --cli`
- Ejecutar pruebas: `python app.py --run-tests`
- Reiniciar base de datos (archivo): `python app.py --reset-db`

Nota: No se requieren dependencias externas para la CLI.
"""
...
# -------------------------------------------------
# Intento de importar GUI (opcional)
# -------------------------------------------------
GUI_AVAILABLE = False
HAS_PYPERCLIP = False
try:
    import customtkinter as ctk  # type: ignore
    from tkinter import messagebox, simpledialog, ttk  # type: ignore
    GUI_AVAILABLE = True
    try:
...
# -----------------------
# Utilidades: password hashing (PBKDF2-HMAC-SHA256)
# -----------------------
...
def init_db() -> None:
    ...
    cur.execute("CREATE INDEX IF NOT EXISTS idx_keyword ON knowledge(keyword)")
...
def register_user_db(username: str, password: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO users (username, salt, passhash) VALUES (?, ?, ?)",
...
# Knowledge CRUD
def query_knowledge_db(keyword: str) -> Optional[str]:
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
...
def list_knowledge_rows() -> List[tuple]:
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
...
def add_knowledge(keyword: str, description: str) -> Tuple[bool, str]:
    ...
    try:
        cur.execute(
            "INSERT INTO knowledge (keyword, description) VALUES (?, ?)",
            (keyword.lower(), description),
        )
        conn.commit()
...
def update_knowledge(row_id: int, keyword: str, description: str) -> Tuple[bool, str]:
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
    try:
        cur.execute(
            "UPDATE knowledge SET keyword = ?, description = ? WHERE id = ?",
            (keyword.lower(), description, row_id),
        )
        conn.commit()
        if cur.rowcount == 0:
...
def delete_knowledge(row_id: int) -> Tuple[bool, str]:
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
...
# -------------------------------------------------
# GUI (solo si hay Tkinter/CustomTkinter disponibles)
# -------------------------------------------------
if GUI_AVAILABLE:
    ...
    class App(ctk.CTk):
        def __init__(self):
            super().__init__()
            self.title("Aplicación de Consulta - Integrador Mejorada")
        ...
        def build_login_ui(self):
            ...
            self.password_entry = ctk.CTkEntry(
                self.login_frame, placeholder_text="Contraseña", show="*"
            )
            self.password_entry.pack(pady=(0, 10), ipady=6, padx=40)
            btn_frame = ctk.CTkFrame(self.login_frame, fg_color="transparent")
            btn_frame.pack(pady=10)
            login_btn = ctk.CTkButton(
                btn_frame, text="Iniciar sesión", command=self.login_action, width=160
            )
            login_btn.grid(row=0, column=0, padx=8)
            reg_btn = ctk.CTkButton(
                btn_frame, text="Registrar", command=self.register_action, width=160
            )
        ...
        def open_main_window(self, username):
            ...
            logout_btn = ctk.CTkButton(
                topbar, text="Cerrar sesión", width=120, command=self.restart_app
            )
            ...
            q_label = ctk.CTkLabel(
                left, text="Ingrese término de consulta", font=ctk.CTkFont(size=16)
            )
            ...
            self.q_entry.bind("<Return>", lambda e: self.perform_query())
            q_btn = ctk.CTkButton(left, text="Buscar", command=self.perform_query)
            q_btn.pack(pady=6)
            clear_btn = ctk.CTkButton(
                left, text="Limpiar", command=lambda: self.q_entry.delete(0, "end")
            )
            clear_btn.pack(pady=6)
            manage_btn = ctk.CTkButton(
                left,
                text="Administrar base de conocimiento",
                command=self.open_manage_knowledge,
            )
            ...
            if HAS_PYPERCLIP:
                copy_btn = ctk.CTkButton(
                    right, text="Copiar resultado", command=self.copy_result
                )
        ...
        def perform_query(self):
            ...
            if res:
                ...
                self.result_text.insert(
                    "0.0", "No se encontraron resultados para: " + term
                )

        def copy_result(self):
            if not HAS_PYPERCLIP:
                messagebox.showwarning(
                    "Aviso", "pyperclip no está disponible en este entorno."
                )
                return
        ...
        def open_manage_knowledge(self):
            ...
            tree.column("description", width=560, anchor="w")
            ...
            def add_entry():
                k = simpledialog.askstring(
                    "Nueva keyword", "Ingrese la keyword (sin espacios):", parent=win
                )
                if k is None:
                    return
                d = simpledialog.askstring(
                    "Descripción", "Ingrese la descripción:", parent=win
                )
                if d is None:
                    return
            ...
            def edit_entry():
                ...
                if not sel:
                    messagebox.showwarning(
                        "Atención", "Seleccione una entrada para editar.", parent=win
                    )
                    return
                item = tree.item(sel[0])
                row_id, keyword, description = item["values"]
                k = simpledialog.askstring(
                    "Editar keyword", "Keyword:", initialvalue=keyword, parent=win
                )
                if k is None:
                    return
                d = simpledialog.askstring(
            ...
            def del_entry():
                ...
                if not sel:
                    messagebox.showwarning(
                        "Atención", "Seleccione una entrada para eliminar.", parent=win
                    )
                    return
                item = tree.item(sel[0])
                row_id = item["values"][0]
                if messagebox.askyesno(
                    "Confirmar", "¿Eliminar la entrada seleccionada?", parent=win
                ):
            ...
            c_close = ctk.CTkButton(
                btn_frame, text="Cerrar", width=120, command=win.destroy
            )
...
# -------------------------------------------------
# CLI (fallback sin dependencias externas)
# -------------------------------------------------
import getpass

def cli_main() -> None:
    print("\n=== Aplicación de Consulta (CLI) ===")
    print("Base de datos:", os.path.abspath(DB_FILE))
    init_db()
...
def cli_session(username: str) -> None:
    while True:
        print(
            "\n--- Menú Principal ---\n"
            "1) Consultar término\n"
        ...
        if opt == "1":
            term = input("Término a buscar: ").strip()
            res = query_knowledge_db(term)
            print(res if res else f"No se encontraron resultados para: {term}")
        elif opt == "2":
            rows = list_knowledge_rows()
            if not rows:
                print("(Sin entradas)")
            else:
                for r in rows:
            ...
            k = input("Nueva keyword: ").strip()
            d = input("Descripción: ").strip()
            ok, msg = add_knowledge(k, d)
            print(("[OK] " if ok else "[ERROR] ") + msg)
        elif opt == "4":
            try:
                row_id = int(input("ID a editar: ").strip())
            except ValueError:
                print("ID inválido")
                continue
            k = input("Nueva keyword: ").strip()
            d = input("Nueva descripción: ").strip()
            ok, msg = update_knowledge(row_id, k, d)
            print(("[OK] " if ok else "[ERROR] ") + msg)
        elif opt == "5":
            try:
                row_id = int(input("ID a eliminar: ").strip())
            except ValueError:
                print("ID inválido")
                continue
            ok, msg = delete_knowledge(row_id)
...
class AppTests(unittest.TestCase):
    def setUp(self):
        ...
        self.tmp.close()
        global DB_FILE
        self._old_db = DB_FILE
        DB_FILE = self.tmp.name
        init_db()
    ...
    def tearDown(self):
        try:
            ...
            pass
        global DB_FILE
        DB_FILE = self._old_db

    def test_user_register_and_login(self):
        ok, msg = register_user_db("alice", "1234")
        self.assertTrue(ok)
        self.assertTrue(check_credentials_db("alice", "1234"))
        self.assertFalse(check_credentials_db("alice", "bad"))
        ok2, _ = register_user_db("alice", "xxxx")
        self.assertFalse(ok2)

    def test_knowledge_crud(self):
        ok, _ = add_knowledge("udp", "User Datagram Protocol")
        self.assertTrue(ok)
        ok2, _ = add_knowledge("udp", "otro")
        self.assertFalse(ok2)
        rows = list_knowledge_rows()
        row = [r for r in rows if r[1] == "udp"][0]
        row_id = row[0]
        self.assertEqual(query_knowledge_db("udp"), "User Datagram Protocol")
        ok3, _ = update_knowledge(row_id, "udp", "UDP protocolo no orientado a conexión")
        self.assertTrue(ok3)
        self.assertIn("no orientado", query_knowledge_db("udp"))
        ok4, _ = delete_knowledge(row_id)
        self.assertTrue(ok4)
        self.assertIsNone(query_knowledge_db("udp"))
...
def main():
    parser = argparse.ArgumentParser(description="Aplicación de Consulta (GUI/CLI)")
    parser.add_argument("--cli", action="store_true", help="Forzar modo CLI")
    parser.add_argument("--run-tests", action="store_true", help="Ejecutar pruebas unitarias")
    parser.add_argument("--reset-db", action="store_true", help="Eliminar archivo de base de datos y recrearlo")
    ...
    if args.reset_db:
        if os.path.exists(DB_FILE):
            os.remove(DB_FILE)
        init_db()
        print("Base de datos reiniciada en:", os.path.abspath(DB_FILE))
        sys.exit(0)
